...
export const SearchPage = () => {
  ...
  return (
    <section className="sm:max-w-[512px] w-full bg-white rounded-[18px] m-auto mt-[100px] p-6">
      ...
      {record ? (
        <div className="mt-4">
          <div className="flex gap-4 justify-between my-3">
            Record Id :<span className="font-semibold"> {record.recordId}</span>
          </div>
          <div className="flex gap-4 justify-between my-3">
            {" "}
            Name :<span className="font-semibold"> {record.patientName}</span>
          </div>
          <div className="flex gap-4 justify-between my-3">
            {" "}
            Date of Birth :
            <span className="font-semibold"> {record.patientDOB}</span>
          </div>
          <div className="flex gap-4 justify-between my-3">
            Gender :
            <span className="font-semibold"> {record.patientGender}</span>
          </div>
          <div className="flex gap-4 justify-between my-3">
            Diagnosis :
            <span className="font-semibold max-w-[300px] text-end">
              {" "}
              {record.diagnosis}
            </span>
          </div>
      ...
      {error.length > 1 && (
        <div className="p-3 bg-red-200 rounded-sm mt-2">{error}</div>
      )}
    </section>
  );
};
